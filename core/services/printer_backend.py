...
def get_printer_backend(printer: PrinterProfile) -> PrinterBackend:
    ...
    Currently only the Klipper/Moonraker backend is implemented.  When
    additional backends (e.g. Bambu Lab) are added, this factory will
    dispatch on a ``printer_type`` field on :class:`PrinterProfile`.
